...
<body>
	<div class="header">
		...
		<p class="top-min d-block center font-weight-light">{{ date('d M, Y', strtotime($transaction->created_at)) }}</p>
		<hr class="hr-dash">
		<table class="w-100">
			<tr>
				<td class="left w-long">Kode Transaksi : </td>
				<td class="right"colspan="3">{{ $transaction->kode_transaksi }}</td>
			...
			<tr>
				<td class="left">Kasir : </td>
					...
					$kasir = $nama_kasir[0];
					@endphp
				<td class="right"colspan="3">{{ $transaction->kasir }}</td>
			</tr>
